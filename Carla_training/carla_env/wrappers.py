...
class Vehicle(CarlaActorBase):
    ...
    def get_body_velocity(self):
        """
        Returns the velocity vector in the vehicle-fixed frame.
        """
        # Get world-frame velocity
        velocity = self.get_velocity()
        v_world = np.array([velocity.x, velocity.y, velocity.z]).reshape(3, 1)

        # Get vehicle yaw angle (rotation around Z-axis)
        yaw = np.deg2rad(self.get_transform().rotation.yaw)

        # Compute the 3x3 rotation matrix for yaw (assuming Z-up frame)
        rot_mat = np.array([
        ...
        v_body = rot_mat @ v_world

        return v_body

    def get_slip_angle(self):
        """
        Returns current slip angle in degrees.
        Positive = velocity points to the left of heading.
        Negative = velocity points to the right of heading.
        """
        velocity = self.get_velocity()
        v_x, v_y = velocity.x, velocity.y
        if np.hypot(v_x, v_y) < 1e-6:
            return 0.0
        yaw = np.deg2rad(self.get_transform().rotation.yaw)
        velocity_angle = np.arctan2(v_y, v_x)
        beta_rad = velocity_angle - yaw 
    ...
    def get_lookahead_slip(self, lookahead_distance):
        """
        Returns lookahead waypoint slip
        """
        transform = self.get_transform()
        current_location = transform.location
        current_waypoint = self.world.map.get_waypoint(current_location, project_to_road = True)
        ...
        while accumulated_distance < lookahead_distance:
            next_waypoint = waypoint.next(1.0)
            if not next_waypoint:
                break
            next_wp = next_waypoint[0]
            segment_length = next_wp.transform.location.distance(waypoint.transform.location)
            accumulated_distance += segment_length
            waypoint = next_wp

        lookahead_yaw = waypoint.transform.rotation.yaw
        heading_rad = np.deg2rad(lookahead_yaw)

        velocity_angle = np.arctan2((v_y + yaw_rate*lookahead_distance), v_x)
        lookahead_slip = np.rad2deg(velocity_angle - heading_rad)

        return lookahead_slip

    def estimate_curvature(self, points):
        if len(points) < 3:
            return 0.0
        p1, p2, p3 = points[0], points[len(points)//2], points[-1]

        # Compute curvature via circle through 3 points
