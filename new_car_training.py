...
class CargoBalancingEnv(gym.Env):
    def __init__(self, xml_file="./car.xml", waypoint_file="./new_curve_waypoints.csv", render_mode=None):
        ...
        large_value = 1e10
        self.observation_space = gym.spaces.Box(low=-large_value, high=large_value, shape=(5,), dtype=np.float32)
        ...
        self.initial_x = 1.27154614e-06
        self.initial_y = -1.68559886e-07
    ...
    def step(self, action):
        ...
        self.data.ctrl[:] = np.array([throttle, steering])  # Send control signals
        ...
        if self.prev_position is not None:
            step_distance = np.linalg.norm(current_position - self.prev_position)
            self.total_distance_covered += step_distance
        ...
        if all(v is not None for v in [lookahead, target_heading, lookahead_distance]):
            #Check early termination
            if not self.early_terminal_state(lat_error, remaining_path_length):
                reward +=self.lookahead_reward(centering_factor,angle_factor,sdf,total_distance_factor)
            else:
                self.terminal_state = True
                reward += penalty
                print("Vehicle reached early termination")
    ...
    def reset(self, seed=SEED, options=None):
        ...
        self.total_distance_covered = 0.0
        self.prev_position = np.array([self.initial_x, self.initial_y])
    ...
    def reward_fn(self, centering_factor, angle_factor, sdf, total_distance_factor):
        # Speed reward
        ...
        reward = speed_reward*centering_factor*angle_factor*sdf*total_distance_factor

        return reward
    
    def lookahead_reward(self, centering_factor, angle_factor, sdf, total_distance_factor):
        test_point = (self.state[0], self.state[1])        
        lookahead_distance, target_heading, lookahead = self.lookahead(test_point)
        _, lateral_distance, closest_point, heading, remaining_path_length = self.shortest_distance_to_spline(test_point)
        ...
        reward = (speed_importance * speed_reward + inclination_importance * inclination_reward) * \
         centering_factor * angle_factor * sdf * total_distance_factor
